export class SchedulerUtils implements IScheduler {
    ...
    getDefaultDate(date) {
        ...
        defaultDate = td.getFullYear().toString() + "-";

        if (mm < 10) {
            defaultDate += '0';
        }
        defaultDate += mm.toString() + "-";

        if (dd < 10) {
            defaultDate += '0';
        }
        defaultDate += dd.toString();
        return defaultDate;
    }
...
export class OneDayBuilder implements ISchedulerBuilder {
    initializeSCheduler() {
        ...
        builder['minTime'] = "07:00:00";

        builder['maxTime'] = "20:00:00";

        builder['enabledEventModification'] = true;
